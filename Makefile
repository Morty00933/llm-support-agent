...
db-reset: ## Reset database (WARNING: destroys all data)
	...
	@docker compose exec postgres sh -c "for i in 1 2 3 4 5; do sleep 1; done" 2>/dev/null || timeout /t 5 /nobreak >NUL 2>&1 || true
...
health: ## Check service health
	@echo "Health Check:"
	@echo -n "  Backend:  " && curl -sf http://localhost:8000/health >/dev/null 2>&1 && echo "OK" || echo "FAIL"
	@echo -n "  Frontend: " && curl -sf http://localhost:3000 >/dev/null 2>&1 && echo "OK" || echo "FAIL"
	@echo -n "  Ollama:   " && curl -sf http://localhost:11434/api/tags >/dev/null 2>&1 && echo "OK" || echo "FAIL"
...
backup-db: ## Backup database to ./backups/
	...
	@docker compose exec -T postgres pg_dump -U postgres llm_agent > "backups/backup_$$(date +%Y%m%d_%H%M%S).sql"
