...
async def create_user(
    session: AsyncSession, tenant_id: int, email: str, password_hash: str
) -> User:
...
async def create_ticket(
    session: AsyncSession, tenant_id: int, title: str, status: str = "open"
) -> Ticket:
...
async def update_ticket_status(
    session: AsyncSession, tenant_id: int, ticket_id: int, status: str
) -> Optional[Ticket]:
...
async def add_message(
    session: AsyncSession, ticket_id: int, role: str, content: str
) -> Message:
...
async def list_messages(
    session: AsyncSession, ticket_id: int, limit: int = 200
) -> Sequence[Message]:
    q = (
        select(Message)
...
async def upsert_kb_chunks(
    ...
    for chunk_text, emb in chunks:
        kc = KBChunk(
            tenant_id=tenant_id, source=source, chunk=chunk_text, embedding=emb
        )
        session.add(kc)
        total += 1
